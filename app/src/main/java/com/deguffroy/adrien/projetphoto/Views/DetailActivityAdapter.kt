...
class DetailActivityAdapter(options: FirestorePagingOptions<Comment>, private val callback:Listener) : FirestorePagingAdapter<Comment, DetailActivityViewHolder>(options) {
    ...
    override fun onLoadingStateChanged(state: LoadingState) {
        ...
        when (state) {
            LoadingState.LOADING_INITIAL -> { Log.e("DetailAdapter","LOADING_INITIAL")}
            LoadingState.LOADING_MORE -> {Log.e("DetailAdapter","LOADING_MORE")}
            LoadingState.LOADED -> {Log.e("DetailAdapter","LOADED")}
            LoadingState.ERROR -> {Log.e("DetailAdapter","ERROR")}
            LoadingState.FINISHED -> {Log.e("DetailAdapter","FINISHED")}
        }
    }
